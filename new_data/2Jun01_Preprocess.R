...
# 1. DATA LOADING =======
cancer_tpm_path <- "../archive/data/pnas_tpm_96_nodup.txt"
cancer_tpm <- read.table(cancer_tpm_path, header = F, row.names = 1, sep = "\t", check.names = FALSE) #96 samples
normal_tpm_path <- "../archive/data/pnas_normal_tpm.txt"
...
common_genes <- intersect(rownames(cancer_tpm), rownames(normal_tpm)) # overlap exactly
...
# 5. GENE SELECTION USING GENE LIST =======
gene_list_path <- "/Users/hazel/Dropbox/desktop/CSE283/proj_CSE283/breast_cancer_classifier/bc_oncogenes.csv"
gene_list <- read.csv(gene_list_path, header = T, stringsAsFactors = FALSE)
target_genes <- gene_list$Ensembl.Gene.ID
genes_in_data <- intersect(target_genes, rownames(norm_log2_tpm))
cat("Total genes in your gene list:", length(target_genes), "\n") # 435
cat("Genes not found:", length(target_genes) - length(genes_in_data), "\n") # 4
setdiff(target_genes, rownames(norm_log2_tpm)) #"ENSG00000100146" "ENSG00000293106" "ENSG00000189409" "ENSG00000213402"

ml_data_onco <- norm_log2_tpm[genes_in_data, ]

cat("Original filtered dataset dimensions:", dim(norm_log2_tpm), "\n") #50379 128
cat("ML dataset dimensions:", dim(ml_data_onco), "\n") #431 128
...
pca_log2_var <- perform_pca_analysis(ml_data_onco, "Log2(TPM+1) - Most Variable", sample_info)

# 7. SAMPLE CORRELATION ANALYSIS=======
sample_cors <- cor(ml_data_onco, method = "pearson")
pheatmap(
  sample_cors,
...
ml_dataset_onco <- prepare_ml_dataset(ml_data_onco, sample_info, "Oncogenes")

# MAIN dataset
write.csv(ml_dataset_onco, "onco_tpm_431genes.csv", row.names = FALSE)
# # gene lists
# write.csv(
...
# ==============================================================================
library(readxl)
validation_tpm_path <- "../archive/data/validation_exon_tpm"
validation_tpm <- read.table(validation_tpm_path, header = TRUE, row.names = 1, sep = "\t", check.names = FALSE)
# Load metadata files
bc_meta_path <- "../archive/data/validation_bc_meta.xlsx"
normal_meta_path <- "../archive/data/validation_normal_meta.xlsx"
...
common_genes_val <- intersect(genes_in_data, validation_genes) #431
